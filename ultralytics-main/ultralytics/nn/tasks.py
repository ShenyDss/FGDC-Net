...
class BaseModel(torch.nn.Module):
    ...
    def loss(self, batch, preds=None):
        ...
        if getattr(head, "use_vfm", False):
            vfm_loss = getattr(head, "vfm_loss", None)
            if vfm_loss is None:
                vfm_cls = loss_items.new_zeros(())
                vfm_reg = loss_items.new_zeros(())
                loss = torch.cat((loss, vfm_cls.view(1), vfm_reg.view(1)))
            else:
                vfm_cls = getattr(head, "vfm_cls_loss", None)
                vfm_reg = getattr(head, "vfm_reg_loss", None)
                vfm_cls = vfm_loss.detach() if vfm_cls is None else vfm_cls.to(loss_items.device)
                vfm_reg = loss_items.new_zeros(()) if vfm_reg is None else vfm_reg.to(loss_items.device)
                loss = torch.cat((loss, vfm_cls.view(1), vfm_reg.view(1)))
            loss_items = torch.cat((loss_items, vfm_cls.detach().view(1), vfm_reg.detach().view(1)))
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in {Detect, FGDCDetect, YOLOEDetect, Segment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26}:
                m.legacy = legacy
        elif m is v10Detect:
