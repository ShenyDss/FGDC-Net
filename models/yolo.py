...
from models.dual_path_head import DualPathDetect
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Segment, DualPathDetect)):
...
def parse_model(d, ch):
    ...
    head_type = d.get("head_type", "detect")
    if act:
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m is Detect and head_type == "dual_path":
            m = DualPathDetect
        ...
        # TODO: channel, gw, gd
        elif m in {Detect, Segment, DualPathDetect}:
            head_kwargs = args.pop(-1) if m is DualPathDetect and args and isinstance(args[-1], dict) else {}
            args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
            if m is Segment:
                args[3] = make_divisible(args[3] * gw, ch_mul)
            if m is DualPathDetect and head_kwargs:
                args.extend(
                    [
                        head_kwargs.get("inplace", True),
                        head_kwargs.get("proj_ratio", 0.5),
                        head_kwargs.get("reduction", 4),
